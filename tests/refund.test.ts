...
import { RefundApi } from "../src/apis/refund.js";
...
describe("RefundApi", () => {
  let resource: RefundApi;

  beforeEach(async () => {
    mockRequest.mockReset();
    const { ApiClient } = await import("../src/core/api-client.js");
    resource = new RefundApi(new ApiClient({} as ApiClientConfig));
  });
  ...
  describe("retry", () => {
    it("calls the correct endpoint on success", async () => {
      mockRequest.mockResolvedValue({ status: true, message: "ok", data: {} });
      await (resource as any).retry(...Array(2).fill("test"));
      expect(mockRequest).toHaveBeenCalledWith(
        expect.stringContaining("POST"),
        expect.any(String),
        expect.anything()
    ...
    it("propagates errors from the HTTP client", async () => {
      mockRequest.mockRejectedValue(new Error("API Error"));
      await expect(
        (resource as any).retry(...Array(2).fill("test"))
      ).rejects.toThrow();
    });
  });
